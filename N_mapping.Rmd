...
df=
  ...
  rename(Ncontent= `N(%)`)
df$Position= factor(df$Position, levels= c("a","ab", "b", "1/4 bc", "1/2 bc"))
```


## N content according to leaf rank

The N content was measured for several leaf ranks. Let's investigate for the effect of the rank on leaf nitrogen content:  
...
The nitrogen content decrease with increasing leaf rank, which is itself a proxy for leaf age. The same effect is observed for both progenies (progeny 5, up and progeny 6, down). This is a well known effect that is due to nitrogen remobilization from old leaves to younger leaves.

## N content according to leaflet position

The N content was measured on five leaflets with different positions along the leaf. Does the position has an effect on the leaf nitrogen content ? 

```{r}
ggplot(data= df, aes(x= Position, y= Ncontent))+
  geom_boxplot(notch = TRUE)+
  facet_grid(vars(Progeny))+
  ylab("N content (%)")+
  xlab("Leaflet position on the leaf")+
  ggtitle("Effect of the leaflet position on N content")
```

The figure above shows that even though the variability seems to decrease from `a` towards `1/2 bc`, the nitrogen content remains quite similar between positions (the notches are almost all overlapping) for both progenies.
...
## Photosynthesis according to leaflet position

The photosynthesis was measured on five leaflets with different positions along the leaf. Does the position has an effect on it ? 

```{r}
ggplot(data= df, aes(x= Position, y= Photo))+
  geom_boxplot(notch = TRUE)+
  ylab("Photosynthesis (umol m-2 s-1)")+
  xlab("Leaflet position on the leaf")+
  facet_grid(vars(Progeny))+
  ggtitle("Effect of the leaflet position on photosynthesis")
```
